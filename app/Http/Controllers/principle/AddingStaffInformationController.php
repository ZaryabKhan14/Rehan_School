...
class AddingStaffInformationController extends Controller
{
    public function show_staffs(){
        $user = Auth::user();
        $campus = $user->campus;
        $show_staff = User::where('campus', $campus)
        ->where(function($query) {
            $query->where('role', 'principle')
                  ->orWhere('role', 'teacher');
        })
        ->get();
        return view('principle.staff',compact('show_staff'));
    }

    public function staffs_information_form($id){
        $staff = User::find($id);
        return view('principle.adding_staff_information_form',compact('staff'));
    }

    public function adding_staff_information(Request $request){
